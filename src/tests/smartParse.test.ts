...
import { parseDay, type SmartContext, type SmartEntry } from '../services/smartParse';
...
function ctx(extra: Partial<SmartContext> = {}): SmartContext {
  return {
    categories, wallets, labels: sys, defaultWalletId: walletId('cash'), baseCurrency: 'MRU',
...
function describe1(e: SmartEntry, c: Category[] = categories): string {
  ...
  const w = (id?: string) => wallets.find((x) => x.id === id)?.sysKey ?? id ?? '';
...
const CASES: Array<[string, string[]]> = [
  ...
  ['خبز 50، تاكسي 100، رصيد 200 من بنكيلي، ودخلتني عمولة 2000',
    ['expense 50 groceries cash', 'expense 100 taxi cash', 'expense 200 phone bankily', 'income 2000 commissions cash']],
  // fusha
  ['اشتريت خبزا بخمسين أوقية', ['expense 50 groceries cash']],
  ['دفعت فاتورة الكهرباء 3500 من مصرفي', ['expense 3500 utilities masrvi']],
  ['استلمت الراتب 45000 في البنك', ['income 45000 salary bank']],
  ['دواء من الصيدلية 1200', ['expense 1200 health cash']],
  ['إيجار الشهر 25000 بالسداد', ['expense 25000 rent sedad']],
  ['صدقة 200', ['expense 200 charity cash']],
  ['غداء في المطعم 800', ['expense 800 restaurants cash']],
  ['كتب المدرسة 1500', ['expense 1500 education cash']],
  // Hassaniya
  ['أتاي 100 وسكر 150', ['expense 100 groceries cash', 'expense 150 groceries cash']],
  ['شريت لحم ب ألف وخمسمية', ['expense 1500 groceries cash']],
  ['ليصانص ميتين', ['expense 200 fuel cash']],
  ['حوت 300 ولبارح تاكسي 100', ['expense 300 groceries cash', 'expense 100 taxi cash d1']],
  ['جاني 5000 من خدمة', ['income 5000 services cash']],
  ['وصلني راتب 60 ألف على بنكيلي', ['income 60000 salary bankily']],
  ['كار 50 ومازوت 2000', ['expense 50 transport cash', 'expense 2000 fuel cash']],
  ['دراعة 4000 من بنكيلي', ['expense 4000 clothes bankily']],
  ['الضو 1500 والكراء 20000', ['expense 1500 utilities cash', 'expense 20000 rent cash']],
  // French
  ['pain 50, taxi 100', ['expense 50 groceries cash', 'expense 100 taxi cash']],
  ['salaire reçu 45000', ['income 45000 salary cash']],
  ['courses 2500 avec bankily', ['expense 2500 groceries bankily']],
  ['hier essence 3000', ['expense 3000 fuel cash d1']],
  ['commission 1500 et crédit 300', ['income 1500 commissions cash', 'expense 300 phone cash']],
  ['pharmacie deux mille', ['expense 2000 health cash']],
  // mixed Arabic / French
  ['taxi 100، خبز 50', ['expense 100 taxi cash', 'expense 50 groceries cash']],
  ['رصيد mattel 500', ['expense 500 phone cash']],
  // numbers
  ['خبز ٥٠', ['expense 50 groceries cash']],
  ['إيجار 25,000', ['expense 25000 rent cash']],
  ['إيجار 25 000', ['expense 25000 rent cash']],
  ['إيجار 25.000', ['expense 25000 rent cash']],
  ['عصير 12.50', ['expense 12.5 restaurants cash']],
  ['رز ب50', ['expense 50 groceries cash']],
  ['رز بـ50', ['expense 50 groceries cash']],
  ['زيت 700 أوقية', ['expense 700 groceries cash']],
  ['زيت 700 MRU', ['expense 700 groceries cash']],
  ['خبز مية وخمسين', ['expense 150 groceries cash']],
  ['كهرباء ثلاث آلاف', ['expense 3000 utilities cash']],
  ['كهرباء 3 آلاف', ['expense 3000 utilities cash']],
  ['راتب مليون ونص', ['income 1500000 salary cash']],
  ['خبز ألف قديمة', ['expense 100 groceries cash']],
  // several entries, income and expense together, amount first
  ['50 خبز 100 تاكسي', ['expense 50 groceries cash', 'expense 100 taxi cash']],
  ['خبز 50\nتاكسي 100\nدخلتني 3000', ['expense 50 groceries cash', 'expense 100 taxi cash', 'income 3000 other_income cash']],
  // transfers
  ['حولت 1000 من بنكيلي للكاش', ['transfer 1000 bankily → cash']],
  ['حولت 5000 لمصرفي', ['transfer 5000 cash → masrvi']],
  ['virement 2000 de bankily vers banque', ['transfer 2000 bankily → bank']],
  // debts
  ['سلفت أحمد 500', ['debt 500 owed_to_me أحمد cash']],
  ['تسلفت من سيدي 2000', ['debt 2000 i_owe سيدي cash']],
  // dates
  ['أمس خبز 50، تاكسي 100', ['expense 50 groceries cash d1', 'expense 100 taxi cash d1']],
  ['أول أمس دواء 900', ['expense 900 health cash d2']],
  ['يوم الخميس غداء 600', ['expense 600 food cash d2']],
  ['اليوم شاي 100', ['expense 100 groceries cash']],
  // tricky: comma without space, French article, weak number words, weekday that is today
  ['خبز 50,تاكسي 100', ['expense 50 groceries cash', 'expense 100 taxi cash']],
  ['une pizza 500', ['expense 500 restaurants cash']],
  ['ست مية ليصانص', ['expense 600 fuel cash']],
  ['السبت خبز 50', ['expense 50 groceries cash']],
  ['خلصت الضو 1500 من بنكيلي', ['expense 1500 utilities bankily']],
  // unknown word: still an entry, in "other", flagged
  ['فلان 300', ['expense 300 other_expense cash']],
];
...
describe('parseDay: details', () => {
  ...
  it('a date alone is not "not understood" and applies to what follows', () => {
    ...
    expect(r.unknown).toEqual([]);
    expect(read('البارح: خبز 50')).toEqual(['expense 50 groceries cash d1']);
  });
  ...
  it('learned words win: "شاي ← عائلة" and a word → wallet', () => {
    ...
    expect(read('شاي 100', c)).toEqual(['expense 100 family cash']);
  ...
  it('archived categories/wallets are never chosen; default wallet falls back to an active one', () => {
    ...
    expect(e.walletId).not.toBe(walletId('cash'));
  });
